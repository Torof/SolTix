...
export default function EventDetailsPage() {
  ...
  useEffect(() => {
    async function fetchEvent() {
      try {
        const eventData = await getEventById(id as string, wallet);
        setEvent(eventData);
      } catch (error) {
  ...
  const handleBuyTicket = async () => {
    ...
    try {
      ...
      // Call the buyTicket service with the wallet
      const result = await buyTicket(id as string, publicKey.toString(), wallet);
            ...
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6">
              <div className="flex">
                <p className="text-sm text-yellow-700">
                  Connect your wallet to view blockchain event data and purchase tickets.
                </p>
              </div>
            </div>
          )}
