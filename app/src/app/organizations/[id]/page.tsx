...
export default function OrganizationDetailsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const orgData = await getOrganizationById(id as string);
        setOrganization(orgData);
        
        if (orgData) {
          // Get events for this organization
          const upcoming = (await getUpcomingEventsMock()).filter(
            event => event.organizationId === orgData.id
          );
          
          const ongoing = (await getOngoingEventsMock()).filter(
            event => event.organizationId === orgData.id
          );
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="relative rounded-xl overflow-hidden mb-8">
        <div className="h-64 overflow-hidden">
          <img 
            src={organization.bannerUrl || 'https://via.placeholder.com/1200x400?text=Organization+Banner'} 
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/80 to-transparent text-white">
          <div className="flex items-center">
            <div className="w-20 h-20 rounded-full overflow-hidden border-4 border-white mr-4">
              <img 
                src={organization.logoUrl || 'https://via.placeholder.com/100?text=Logo'} 
